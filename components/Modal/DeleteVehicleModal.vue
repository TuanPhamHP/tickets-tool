<template>
	<UModal
		...
		:dismissible="false"
		:close="false"
		title="Xác nhận xoá thông tin xe"
	>
		<template #body>
			<div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
				<div class="mb-2">
					<p class="text-md text-center text-gray-700">
						Bạn có chắc chắn muốn xoá thông tin xe:
						<br />
		...
		methods: {
			handleDelete() {
				...
				this.apiService.vehicle
					...
					.then(
						res => {
							this.successToast({ title: `Xoá xe ${this.vehicle?.plate_number} thành công!` });
							this.$emit('deleted');
						},
						err => {
							this.errorToast({ title: `Lỗi xoá xe: ${err}` });
						}
					)
					.finally(() => {
