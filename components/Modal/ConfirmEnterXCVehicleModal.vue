<template>
	<UModal
		...
		:dismissible="false"
		title="Xác nhận xe đã vào bãi Xuân Cương"
		:close="false"
	>
		<template #body>
		...
		methods: {
			handleSubmit() {
				...
				this.apiService.vehicle
					.confirmVehicleEnterXC(this.vehicle.id)
					.then(
						res => {
							this.successToast({ title: `Xe ${this.vehicle?.plate_number} được xác nhận vào bãi XC!` });
							this.$emit('updated');
						},
						err => {
